apiURL = "http://51.210.242.232/api/movies"
    ...
    methods: {
        registerUser: function(event){
            ...
            var data = {
                '_links':{
                    'type' : {
                        'href' : 'http://51.210.242.232/rest/type/user/user'
                    }
                },
                'name':[
                    {
            ...
            this.$http.post('http://51.210.242.232/entity/user', data, function(response){
    ...
    methods: {
        ...
        updateTheMovie: function(event){
            ...
            var data = {
                '_links':{
                    'type' : {
                        'href' : 'http://51.210.242.232/rest/type/node/movies'
                    }
                },
                'title':[
                    {
            ...
            this.$http.patch('http://51.210.242.232/node/' + this.$route.params.movieID, data, function(response){
...
var deleteMovie = Vue.extend({
    ...
    methods:{
        deleteTheMovie: function(){
            this.$http.delete('http://51.210.242.232/node/' + this.$route.params.movieID, function(response){
                this.$route.router.go('/');
            })
        }
    }
...
var createMovie = Vue.extend({
    ...
    methods: {
        createTheMovie: function(event){
            ...
            this.$http.post('http://51.210.242.232/entity/node', data, function(response){
                this.$set('success', 'ok');
                this.$set('title', '');
                this.$set('body', '');
            });
